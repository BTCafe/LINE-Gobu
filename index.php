...
require_once( __DIR__ . '/conf/youtube_setup.php');	
						...
						$id_to_search = $splitted_url[3];
							...
							///////////////////////
								...
								switch ($command) {
									...
									if ($used_points < $current_casino['CURRENT_VALUE_CASINO'] && 0 < $used_points) {
										$result = rand(1,2);
										...
										$text_response = "Casino doesn't have enough points or can't bet with that number\nYou can upgrade Casino for more daily points";
										$display->single_text_response($client, $event, $text_response);
									} 
									break;
							...
							//////////////////////
								...
								// Utility Function
									case '..new':
									$Gobu_API = $youtube_api;

									$option = array (
										'part' => 'snippet',
										'channelId' => 'UC1twkjitJytUYTs3UTL2bDQ',
										'order' => 'date',
										'key' => $Gobu_API
									);
									...
									curl_setopt($ch, CURLOPT_URL, "https://www.googleapis.com/youtube/v3/search?" . http_build_query($option, 'a', '&'));
									...
									$video_details = $video_array['items'][0];
									$video_link = "https://youtu.be/" . $video_details['id']['videoId'] ;
									$video_msg = $video_details['snippet']['title'] ;

									$display->double_text_response($client, $event, $video_link, $video_msg);
									break;
									...
									switch ($exploded_Message[1]) {
										...
										case 'ssg':
										$ori = 'https://i.imgur.com/YLSMw87.jpg' ;
										$ori_preview = 'https://i.imgur.com/YLSMw87m.jpg' ;
										$response = array($ori, $ori_preview); 
										break;
									}
								...
								// Admin Function //
									...
									if ($event['source']['userId'] == 'Uc7871461db4f5476b1d83f71ee559bf0') {
										...
										$total_cost = $exploded_Message[2];
										if ($total_cost <= 0) {
											$display->single_text_response($client, $event, "Please use positive number");
										} else {
											$current_points = $database->get_points($event['source'], $db);
											if ($current_points <= $total_cost) {
												$text_response = sprintf("You don't have enough points to do that") ;
												$display->single_text_response($client, $event, $text_response);
											} else {
												$database->modify_points($event['source'], $db, $total_cost, 0);
												$database->create_coupon($exploded_Message[1], $exploded_Message[2], $db);
